...
// How to change the stream buffer size
const inputStream = fs.createReadStream('400mb.txt', { highWaterMark: 64 * 1024 })
...
const transformer = new Transform({
    ...
    transform(chunk, encoding, callback) {
        console.log(`Reading ${chunk.length} bytes at a time`)
        try {
            const data = chunk.toString()
            let words = data.split(' ')

            // console.log(`Data buffer: ${chunk.toString().length}`)
